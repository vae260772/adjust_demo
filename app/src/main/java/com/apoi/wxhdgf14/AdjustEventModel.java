package com.apoi.wxhdgf14;

public class AdjustEventModel {
    /**
     * 明文字符串最好加密一下，aes、des
     *  "key"、"url"......
     *
     *
     *
     *
     *
     *
     *
     *
     *
     *
     */
    public static final String adjust_key = "key";
    public static final String app_url_key = "url";

    public static final String register_success_key = "register_success";//4l8uel
    public static final String recharge_success_key = "recharge_success";//vvvaoz
    public static final String first_recharge_success_key = "first_recharge_success";//fhbhud


    public static String register_success = "";
    public static String recharge_success = "";
    public static String first_recharge_success = "";
    public static String app_url = "";

}
